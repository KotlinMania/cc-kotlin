# Claude Code Project Instructions — cc-kotlin

## Project Overview

This is **cc-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`cc`](https://crates.io/crates/cc).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/cc/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/cc>
