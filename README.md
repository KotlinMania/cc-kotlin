...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`cc`](https://crates.io/crates/cc).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `cc` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

C/C++ compiler driver for build scripts
...
Original work copyrighted by the upstream `cc` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
